...
namespace NesZord.Tests.New.OpCodes.DEX
{
	public class When_process_DEX_should : When_process_opcode_should
	{
		...
		public void Decrement_1_to_x_register()
		{
			...
			// Assert
			this.Processor.X.Value.Should().Equals(0x04);
		}
		...
		{
			this.Processor.RunProgram(new byte[] { (byte) OpCode.DEX_Implied });
		}
	}
}
